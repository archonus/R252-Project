...
def get_mnist_dataloaders(data_dir: str = '~/data', batch_size: int = 1, train_frac = 0.8, drop_remainder: bool = True):
    """
    ...
    # Calculate the training percentage dynamically
    if not (0.0 <= train_frac < 1.0):
        raise ValueError("train_frac must be strictly between 0.0 and 1.0")
    
    train_pct = int(train_frac * 100)

    # Load datasets
    
    # Construct the split strings (e.g., 'train[:80%]' and 'train[80%:]')
    train_split_str = f'train[:{train_pct}%]'
    val_split_str = f'train[{train_pct}%:]'

    # Load datasets using the dynamic strings
    train_dataset, val_dataset, test_dataset = tfds.load(
        name='mnist',
        split=[train_split_str, val_split_str, 'test'], 
        as_supervised=True, 
        data_dir=data_dir, 
        shuffle_files=True
    )
